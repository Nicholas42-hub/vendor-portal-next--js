  ...
  try {
    ...
    if (data) {
      ...
      if (data.dadim_BC_Category_Booking_Forms) {
        saveToStorage(CATEGORIES_STORAGE_KEY, data.dadim_BC_Category_Booking_Forms);
      }
    }
...
export async function fetchVendorsData(accessToken: string) {
  try {
    ...
    return {
      success: false,
      message: `Error fetching vendors data: ${error instanceof Error ? error.message : String(error)}`
    };
  }
}